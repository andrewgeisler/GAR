gaRequest <-
  function(id, dimensions=NA, metrics, start, end, token=NA, sort=NA, max=1000, segment=NA, filters=NA, allResults=FALSE) {
    ...
    queryList <- buildQuery(id, dimensions, metrics, start, end, token, sort, max, segment, filters)
    ...
    if (length(colnames(finalDf))==3) {
      ...
      if (allResults==TRUE & as.numeric(max)==10000 & finalDf$totalResults[1]>10000) {
        ...
        queryListPaginate <- lapply(totalResults$profileId, function(x) { queryList[[x]] } )
